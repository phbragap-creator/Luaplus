...
// Global Big Picture Mode Detector for LuaTools
function _globalIsBigPictureMode() {
  const htmlClasses = document.documentElement.className || "";
  const userAgent = navigator.userAgent || "";

  const hasBigPictureClass = htmlClasses.includes("BasicUI") || htmlClasses.includes("gamepadui") || htmlClasses.includes("GamepadUI") || htmlClasses.includes("tenfoot");
  ...
  const isSteamLoopback = window.location.hostname === "steamloopback.host";
  ...
  if (isSteamLoopback && !hasDesktopClass) bigPictureScore += 2;

  return bigPictureScore > 0;
}

// Define the dynamic getter on window so it's always evaluated correctly even if loaded early
Object.defineProperty(window, "__LUATOOLS_IS_BIG_PICTURE__", {
  get: function () {
    return _globalIsBigPictureMode();
  },
  configurable: true,
  enumerable: true
});
...
(function () {
  ...
  function scanFocusableElements() {
    ...
    // Only scan if there's a LuaTools overlay active (priority-based to enforce Modal Trap on alerts)
    ...
    if (!activeOverlay) {
      activeOverlay = document.querySelector(OVERLAY_SELECTOR_STRING);
    }
    ...
    if (state.focusableElements.length > 0) {
      ...
      let targetIndex = 0;
      if (currentActive) {
        const activeIdx = state.focusableElements.indexOf(currentActive);
        if (activeIdx !== -1) {
  ...
  function navigate(direction) {
    // If a text input is active, freeze navigation to prevent dismissing the Steam virtual keyboard
    const activeEl = document.activeElement;
    if (activeEl) {
      const tagName = activeEl.tagName ? activeEl.tagName.toLowerCase() : "";
      const isTextInput = tagName === "textarea" || (tagName === "input" && (activeEl.type === "text" || activeEl.type === "search"));
  ...
  function handleButtonPress(buttonIndex) {
    // If a text input is active, freeze button mapping to prevent dismissing the Steam virtual keyboard
    const activeEl = document.activeElement;
    if (activeEl) {
      const tagName = activeEl.tagName ? activeEl.tagName.toLowerCase() : "";
      const isTextInput = tagName === "textarea" || (tagName === "input" && (activeEl.type === "text" || activeEl.type === "search"));
    ...
    switch (buttonIndex) {
      case CONFIG.buttonMap.A:
        if (element) {
          ...
          const isTextInput = tagName === "textarea" || (tagName === "input" && (element.type === "text" || element.type === "search"));
          if (!isTextInput) {
...
(function () {
  ...
  // Big Picture Mode Detector - simply check the global getter
  function isBigPictureMode() {
    return window.__LUATOOLS_IS_BIG_PICTURE__;
  }
  ...
  function generateThemeStyles(theme) {
    return `
            ...
            .luatools-header-button img,
            .luatools-header-button svg {
                height: 16px;
                width: 16px;
            }
        `;
  ...
  function showSettingsPopup() {
    ...
    ensureTranslationsLoaded(false)
      ...
      .finally(function () {
        ...
        const savesMenuBtn = createCardButton(
          "lt-settings-saves-menu",
          "menu.savesMenu",
          "Saves Manager",
          "fa-floppy-disk",
        );
        if (isGamePage) cardGrid.appendChild(savesMenuBtn);
        ...
        if (savesMenuBtn) {
          savesMenuBtn.addEventListener("click", function (e) {
            e.preventDefault();
            try {
              overlay.remove();
            } catch (_) {}
            showSavesManagerPopup(appid, showSettingsPopup);
          });
        }
  ...
  function showSavesManagerPopup(appid, onBack) {
    if (document.querySelector(".luatools-saves-overlay")) return;
    ...
    const isBP = isBigPictureMode();
    const colors = getThemeColors();

    const overlay = document.createElement("div");
    overlay.className = "luatools-saves-overlay";
    
    if (isBP) {
      overlay.style.cssText =
        "position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(16px);z-index:100000;display:flex;align-items:center;justify-content:center;font-family:'Motiva Sans', Arial, Helvetica, sans-serif;";
    } else {
      overlay.style.cssText =
        "position:fixed;inset:0;background:rgba(0,0,0,0.78);backdrop-filter:blur(8px);z-index:100000;display:flex;align-items:center;justify-content:center;";
    }

    const modal = document.createElement("div");
    
    if (isBP) {
      modal.style.cssText = `position:relative;background:radial-gradient(circle at 50% 0%, #1e2838 0%, #0f1622 100%);color:#fff;border:1px solid rgba(102, 192, 244, 0.2);border-radius:12px;width:600px;max-height:88vh;padding:24px;display:flex;flex-direction:column;gap:20px;box-shadow:0 24px 80px rgba(0,0,0,.9), inset 0 1px 0 rgba(255,255,255,0.05);animation:slideUp 0.12s ease-out;overflow:hidden;`;
    } else {
      modal.style.cssText = `position:relative;background:${colors.modalBg};color:${colors.text};border:1px solid ${colors.border};border-radius:16px;width:480px;max-height:85vh;padding:20px 24px;display:flex;flex-direction:column;gap:16px;box-shadow:0 16px 40px rgba(0,0,0,0.5);animation:slideUp 0.12s ease-out;overflow:hidden;`;
    }

    // Header
    const header = document.createElement("div");
    if (isBP) {
      header.style.cssText = "display:flex;justify-content:space-between;align-items:center;padding-bottom:14px;border-bottom:1px solid rgba(255,255,255,0.1);";
    } else {
      header.style.cssText = `display:flex;justify-content:space-between;align-items:center;padding-bottom:12px;border-bottom:1px solid ${colors.borderRgba};`;
    }

    const title = document.createElement("div");
    if (isBP) {
      title.style.cssText = "font-size:22px;color:#fff;font-weight:600;display:flex;align-items:center;gap:10px;";
      title.innerHTML = `<i class="fa-solid fa-floppy-disk" style="color:#66c0f4;"></i><span>` + t("saves.title", "Saves Backup & Restore") + `</span>`;
    } else {
      title.style.cssText = `font-size:20px;color:${colors.text};font-weight:600;display:flex;align-items:center;gap:8px;`;
      title.innerHTML = `<i class="fa-solid fa-floppy-disk" style="color:${colors.accent};"></i><span>` + t("saves.title", "Saves Backup & Restore") + `</span>`;
    }

    const closeBtn = document.createElement("a");
    closeBtn.href = "#";
    if (isBP) closeBtn.className = "focusable";
    
    if (isBP) {
      closeBtn.style.cssText = `display:flex;align-items:center;justify-content:center;width:36px;height:36px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#66c0f4;font-size:16px;text-decoration:none;transition:all 0.2s ease;cursor:pointer;`;
      ...
      closeBtn.style.cssText = `display:flex;align-items:center;justify-content:center;width:32px;height:32px;background:rgba(${colors.rgbString},0.08);border:1px solid ${colors.borderRgba};border-radius:8px;color:${colors.accent};font-size:14px;text-decoration:none;transition:all 0.2s ease;cursor:pointer;`;
      closeBtn.onmouseover = function () {
        this.style.background = `rgba(${colors.rgbString},0.18)`;
        this.style.borderColor = colors.accent;
      };
      closeBtn.onmouseout = function () {
        this.style.background = `rgba(${colors.rgbString},0.08)`;
    ...
    if (isBP) {
      content.style.cssText = "flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:20px;padding-right:4px;";
    } else {
      content.style.cssText = "flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:16px;padding-right:4px;";
    }

    // Keyboard tip helper removed as Steam native overlay handles inputs automatically
    const keyboardTip = { style: {} };

    // 1. Save Path Section
    const pathSection = document.createElement("div");
    if (isBP) {
      pathSection.style.cssText = "background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:16px;display:flex;flex-direction:column;gap:12px;";
    } else {
      pathSection.style.cssText = `background:rgba(${colors.rgbString},0.03);border:1px solid ${colors.borderRgba};border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:10px;`;
    }

    const pathTitle = document.createElement("div");
    if (isBP) {
      pathTitle.style.cssText = "font-size:14px;font-weight:600;color:#fff;display:flex;align-items:center;gap:8px;";
      pathTitle.innerHTML = `<i class="fa-solid fa-folder-open" style="color:#66c0f4;"></i><span>` + t("saves.pathTitle", "Save Folder Location") + `</span>`;
    } else {
      pathTitle.style.cssText = `font-size:13px;font-weight:600;color:${colors.text};display:flex;align-items:center;gap:6px;`;
      pathTitle.innerHTML = `<i class="fa-solid fa-folder-open" style="color:${colors.accent};"></i><span>` + t("saves.pathTitle", "Save Folder Location") + `</span>`;
    }
    pathSection.appendChild(pathTitle);

    const currentPathText = document.createElement("div");
    if (isBP) {
      currentPathText.style.cssText = "font-size:12px;font-family:monospace;background:rgba(0,0,0,0.4);padding:10px 12px;border-radius:6px;word-break:break-all;color:#a3b0bd;border:1px solid rgba(255,255,255,0.02);";
    } else {
      currentPathText.style.cssText = `font-size:11px;font-family:monospace;background:rgba(0,0,0,0.15);padding:8px 10px;border-radius:6px;word-break:break-all;color:${colors.textSecondary};border:1px solid rgba(255,255,255,0.02);`;
    }
    currentPathText.textContent = t("saves.loading", "Loading active save path...");
    pathSection.appendChild(currentPathText);

    const pathStatus = document.createElement("div");
    pathStatus.style.cssText = isBP ? "font-size:11px;color:#8f98a0;" : `font-size:11px;color:${colors.textSecondary};`;
    pathSection.appendChild(pathStatus);

    // Path override form
    const overrideForm = document.createElement("div");
    overrideForm.style.cssText = "display:flex;gap:10px;align-items:center;margin-top:2px;";

    const pathInputContainer = document.createElement("div");
    pathInputContainer.style.cssText = "flex:1;position:relative;display:flex;align-items:center;";

    const pathInput = document.createElement("input");
    pathInput.type = "text";
    if (isBP) pathInput.className = "focusable";
    pathInput.placeholder = t("saves.placeholder", "Paste custom saves folder path...");
    
    if (isBP) {
      pathInput.style.cssText = "width:100%;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.1);border-radius:6px;color:#fff;padding:10px 32px 10px 12px;font-size:12px;outline:none;transition:all 0.2s;";
      pathInput.onfocus = function () {
        this.style.borderColor = "#66c0f4";
      };
      pathInput.onblur = function () {
        this.style.borderColor = "rgba(255,255,255,0.1)";
      };
    } else {
      pathInput.style.cssText = `width:100%;background:rgba(0,0,0,0.25);border:1px solid ${colors.borderRgba};border-radius:6px;color:${colors.text};padding:8px 12px;font-size:12px;outline:none;transition:all 0.2s;`;
      pathInput.onfocus = function () {
        this.style.borderColor = colors.accent;
      };
      pathInput.onblur = function () {
    ...
    if (isBP) {
      const pathKeyIcon = document.createElement("i");
      pathKeyIcon.className = "fa-solid fa-keyboard";
      pathKeyIcon.style.cssText = "position:absolute;right:10px;color:rgba(255,255,255,0.3);font-size:12px;pointer-events:none;";
      pathInputContainer.appendChild(pathKeyIcon);
    }

    const savePathBtn = document.createElement("a");
    savePathBtn.href = "#";
    if (isBP) savePathBtn.className = "focusable";
    
    if (isBP) {
      savePathBtn.style.cssText = "background:#66c0f4;color:#000;border-radius:6px;padding:10px 16px;font-size:12px;font-weight:700;text-decoration:none;transition:all 0.2s;cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:6px;";
      savePathBtn.onmouseover = function () {
        this.style.filter = "brightness(1.15)";
      };
      savePathBtn.onmouseout = function () {
        this.style.filter = "brightness(1)";
      };
    } else {
      savePathBtn.style.cssText = `background:${colors.accent};color:#fff;border-radius:6px;padding:8px 14px;font-size:12px;font-weight:600;text-decoration:none;transition:all 0.2s;cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:6px;`;
      savePathBtn.onmouseover = function () {
    ...
    savePathBtn.innerHTML = `<i class="fa-solid fa-floppy-disk"></i><span>` + t("saves.save", "Set Path") + `</span>`;

    overrideForm.appendChild(pathInputContainer);
    overrideForm.appendChild(savePathBtn);
    pathSection.appendChild(overrideForm);
    content.appendChild(pathSection);

    // 2. Search & Filters & Backups Section
    const listSection = document.createElement("div");
    listSection.style.cssText = "display:flex;flex-direction:column;gap:14px;flex:1;";

    const listHeader = document.createElement("div");
    listHeader.style.cssText = "display:flex;justify-content:space-between;align-items:center;";

    const listTitle = document.createElement("div");
    if (isBP) {
      listTitle.style.cssText = "font-size:14px;font-weight:600;color:#fff;display:flex;align-items:center;gap:8px;";
      listTitle.innerHTML = `<i class="fa-solid fa-clock-rotate-left" style="color:#66c0f4;"></i><span>` + t("saves.backupsTitle", "Available Backups") + `</span>`;
    } else {
      listTitle.style.cssText = `font-size:13px;font-weight:600;color:${colors.text};display:flex;align-items:center;gap:6px;`;
      listTitle.innerHTML = `<i class="fa-solid fa-history" style="color:${colors.accent};"></i><span>` + t("saves.backupsTitle", "Available Backups") + `</span>`;
    }
    listHeader.appendChild(listTitle);

    const createBackupBtn = document.createElement("a");
    createBackupBtn.href = "#";
    if (isBP) createBackupBtn.className = "focusable";
    
    if (isBP) {
      createBackupBtn.style.cssText = "background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:6px;padding:8px 14px;font-size:12px;font-weight:700;text-decoration:none;transition:all 0.2s;cursor:pointer;display:flex;align-items:center;gap:6px;";
      createBackupBtn.onmouseover = function () {
        this.style.background = "rgba(102, 192, 244, 0.15)";
        this.style.borderColor = "#66c0f4";
      };
      createBackupBtn.onmouseout = function () {
        this.style.background = "rgba(255,255,255,0.05)";
        this.style.borderColor = "rgba(255,255,255,0.1)";
      };
    } else {
      createBackupBtn.style.cssText = `background:rgba(${colors.rgbString},0.08);border:1px solid ${colors.borderRgba};color:${colors.text};border-radius:6px;padding:6px 12px;font-size:11px;font-weight:600;text-decoration:none;transition:all 0.2s;cursor:pointer;display:flex;align-items:center;gap:6px;`;
      createBackupBtn.onmouseover = function () {
        this.style.background = `rgba(${colors.rgbString},0.15)`;
        this.style.borderColor = colors.accent;
      };
      createBackupBtn.onmouseout = function () {
        this.style.background = `rgba(${colors.rgbString},0.08)`;
    ...
    createBackupBtn.innerHTML = `<i class="fa-solid fa-circle-plus"></i><span>` + t("saves.createBackup", "Backup Now") + `</span>`;
    
    listHeader.appendChild(createBackupBtn);
    listSection.appendChild(listHeader);

    // Search bar completely removed as backups are already sorted chronologically

    // Filters row
    const filtersRow = document.createElement("div");
    filtersRow.style.cssText = "display:flex;gap:8px;margin-bottom:2px;";

    let activeFilter = "all";
    const filterButtons = [];

    function createFilterBtn(id, text, filterValue) {
      const btn = document.createElement("a");
      btn.href = "#";
      if (isBP) btn.className = "focusable";
      
      if (isBP) {
        btn.style.cssText = "background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:6px;padding:6px 12px;color:#8f98a0;font-size:11px;font-weight:700;text-decoration:none;transition:all 0.2s;cursor:pointer;";
      } else {
        btn.style.cssText = `background:rgba(${colors.rgbString},0.03);border:1px solid ${colors.borderRgba};border-radius:6px;padding:5px 10px;color:${colors.textSecondary};font-size:11px;font-weight:600;text-decoration:none;transition:all 0.2s;cursor:pointer;`;
      }
      ...
      btn.addEventListener("click", function (e) {
        ...
        filterButtons.forEach(b => {
          if (b.filterValue === activeFilter) {
            if (isBP) {
              b.el.style.background = "#66c0f4";
              b.el.style.color = "#000";
              b.el.style.borderColor = "#66c0f4";
            } else {
              b.el.style.background = colors.accent;
              b.el.style.color = "#fff";
              b.el.style.borderColor = colors.accent;
            }
          } else {
            if (isBP) {
              b.el.style.background = "rgba(255,255,255,0.03)";
              b.el.style.color = "#8f98a0";
              b.el.style.borderColor = "rgba(255,255,255,0.06)";
            } else {
              b.el.style.background = `rgba(${colors.rgbString},0.03)`;
              b.el.style.color = colors.textSecondary;
              b.el.style.borderColor = colors.borderRgba;
            }
          }
        });
      ...
      filterButtons.push({ el: btn, filterValue: filterValue });
      return btn;
    }

    createFilterBtn("lt-filter-all", t("saves.filterAll", "All"), "all");
    createFilterBtn("lt-filter-24h", t("saves.filter24h", "Last 24h"), "24h");
    createFilterBtn("lt-filter-7d", t("saves.filter7d", "This Week"), "7d");
    createFilterBtn("lt-filter-large", t("saves.filterLarge", "Large (>500KB)"), "large");

    listSection.appendChild(filtersRow);

    // Backups List Container
    const backupsList = document.createElement("div");
    if (isBP) {
      backupsList.style.cssText = "border:1px solid rgba(255,255,255,0.06);border-radius:10px;background:rgba(0,0,0,0.3);min-height:150px;max-height:260px;overflow-y:auto;display:flex;flex-direction:column;padding:8px;gap:8px;";
    } else {
      backupsList.style.cssText = `border:1px solid ${colors.borderRgba};border-radius:10px;background:rgba(0,0,0,0.12);min-height:140px;max-height:220px;overflow-y:auto;display:flex;flex-direction:column;padding:6px;gap:6px;`;
    }
    listSection.appendChild(backupsList);
    ...
    function renderFilteredBackups() {
      backupsList.innerHTML = "";
      const now = Date.now();

      const filtered = allBackups.filter(function (backup) {
        // Quick Filters
        if (activeFilter === "24h") {
          return (now - backup.timestamp * 1000) < 24 * 60 * 60 * 1000;
        }
        if (activeFilter === "7d") {
          return (now - backup.timestamp * 1000) < 7 * 24 * 60 * 60 * 1000;
        }
        if (activeFilter === "large") {
          return backup.size.includes("MB") || (backup.size.includes("KB") && parseFloat(backup.size) >= 500.0);
        }
      ...
      if (filtered.length > 0) {
        filtered.forEach(function (backup) {
          ...
          if (isBP) {
            item.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:rgba(30, 37, 48, 0.45);border:1px solid rgba(255,255,255,0.04);border-radius:8px;transition:all 0.2s ease;";
          } else {
            item.style.cssText = `display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:rgba(${colors.rgbString},0.03);border:1px solid ${colors.borderRgba};border-radius:6px;transition:all 0.2s ease;`;
          }
          ...
          if (isBP) {
            dateText.style.cssText = "font-size:13px;font-weight:700;color:#fff;";
          } else {
            dateText.style.cssText = `font-size:12px;font-weight:600;color:${colors.text};`;
          }
          dateText.textContent = backup.date;

          const sizeText = document.createElement("div");
          if (isBP) {
            sizeText.style.cssText = "font-size:10px;color:#8f98a0;font-family:monospace;";
          } else {
            sizeText.style.cssText = `font-size:10px;color:${colors.textSecondary};font-family:monospace;`;
          }
          sizeText.textContent = `${backup.filename} (${backup.size})`;
          ...
          restoreBtn.href = "#";
          if (isBP) restoreBtn.className = "focusable";
          
          if (isBP) {
            restoreBtn.style.cssText = "background:#2a475e;border:1px solid #38607d;color:#66c0f4;border-radius:4px;padding:7px 14px;font-size:11px;font-weight:700;text-decoration:none;transition:all 0.2s ease;cursor:pointer;display:flex;align-items:center;gap:4px;";
            restoreBtn.onmouseover = function () {
              this.style.background = "#38607d";
              this.style.color = "#fff";
              this.style.borderColor = "#66c0f4";
            };
            restoreBtn.onmouseout = function () {
              this.style.background = "#2a475e";
              this.style.color = "#66c0f4";
              this.style.borderColor = "#38607d";
            };
          } else {
            restoreBtn.style.cssText = `background:rgba(71,161,255,0.1);border:1px solid rgba(71,161,255,0.25);color:#47a1ff;border-radius:4px;padding:6px 10px;font-size:11px;font-weight:600;text-decoration:none;transition:all 0.2s ease;cursor:pointer;display:flex;align-items:center;gap:4px;`;
            restoreBtn.onmouseover = function () {
              this.style.background = "rgba(71,161,255,0.22)";
              this.style.borderColor = "#47a1ff";
            };
            restoreBtn.onmouseout = function () {
              this.style.background = "rgba(71,161,255,0.1)";
              this.style.borderColor = "rgba(71,161,255,0.25)";
            };
          }
          restoreBtn.innerHTML = `<i class="fa-solid fa-rotate-left"></i><span>` + t("saves.restore", "Restore") + `</span>`;

          restoreBtn.onclick = function (e) {
            e.preventDefault();
            const confirmMsg = t("saves.confirmRestore", "Restoring this backup will overwrite the current saves. Proceed?");
            showLuaToolsConfirm(t("saves.title", "Saves Backup & Restore"), confirmMsg, function () {
              // Pre-load spinner alert
              const loadingOverlay = document.createElement("div");
              loadingOverlay.className = "luatools-alert-overlay";
              loadingOverlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(8px);z-index:110000;display:flex;align-items:center;justify-content:center;";
              
              const spinner = document.createElement("div");
              if (isBP) {
                spinner.style.cssText = `background:radial-gradient(circle at 50% 0%, #1e2838 0%, #0f1622 100%);border:1px solid rgba(102, 192, 244, 0.2);border-radius:12px;padding:24px 32px;display:flex;flex-direction:column;align-items:center;gap:12px;box-shadow:0 12px 40px rgba(0,0,0,0.6);`;
                spinner.innerHTML = `<i class="fa-solid fa-spinner fa-spin" style="font-size:32px;color:#66c0f4;"></i><div style="font-size:14px;color:#fff;font-weight:600;">` + t("saves.restoring", "Restoring saves...") + `</div>`;
              } else {
                spinner.style.cssText = `background:${colors.modalBg};border:1px solid ${colors.border};border-radius:12px;padding:20px 28px;display:flex;flex-direction:column;align-items:center;gap:10px;box-shadow:0 8px 30px rgba(0,0,0,0.4);`;
                spinner.innerHTML = `<i class="fa-solid fa-spinner fa-spin" style="font-size:28px;color:${colors.accent};"></i><div style="font-size:13px;color:${colors.text};font-weight:600;">` + t("saves.restoring", "Restoring saves...") + `</div>`;
              }
              
              loadingOverlay.appendChild(spinner);
              document.body.appendChild(loadingOverlay);

              Millennium.callServerMethod("luatools", "RestoreSaveBackup", {
                appid: appid,
                ...
                try {
                  ...
                  const payload2 = typeof restoreRes === "string" ? JSON.parse(restoreRes) : restoreRes;
                  if (payload2 && payload2.success) {
                    ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), t("saves.restoreSuccess", "Save files restored successfully!"));
                  } else {
                    const errMsg = payload2 && payload2.error ? payload2.error : t("saves.restoreFail", "Restoration failed.");
                    ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), errMsg);
                  }
                } catch (_) {
                  loadingOverlay.remove();
                }
                ...
                loadingOverlay.remove();
                ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), String(err2));
              });
            });
          };
        ...
        if (isBP) {
          backupsList.innerHTML = `<div style="text-align:center;padding:32px 12px;color:#8f98a0;font-size:12px;"><i class="fa-solid fa-circle-info" style="margin-right:6px;color:#66c0f4;"></i>` + t("saves.noFilteredBackups", "No backups match the active filters.") + `</div>`;
        } else {
          backupsList.innerHTML = `<div style="text-align:center;padding:24px 12px;color:${colors.textSecondary};font-size:12px;"><i class="fa-solid fa-info-circle" style="margin-right:6px;color:${colors.accent};"></i>` + t("saves.noFilteredBackups", "No backups match the active filters.") + `</div>`;
        }
      }

      // Re-scan gamepad navigation elements (Only in Big Picture)
      if (isBP) {
        setTimeout(function () {
    ...
    // Search input listener completely removed

    // Callbacks & Logic
    function refreshSavePath() {
      if (typeof Millennium === "undefined" || typeof Millennium.callServerMethod !== "function") {
        currentPathText.textContent = "C:/Program Files (x86)/Steam/userdata/.../saves";
        pathStatus.textContent = "Auto-detected (mock)";
        return;
      }

      Millennium.callServerMethod("luatools", "GetSavePathsForApp", {
        appid: appid,
        contentScriptQuery: "",
      }).then(function (res) {
        try {
          ...
          if (payload && payload.success && payload.path) {
            currentPathText.textContent = payload.path;
            pathInput.value = payload.path;
            if (payload.isManual) {
              pathStatus.innerHTML = `<span style="color:#f5a623;"><i class="fa-solid fa-user-gear"></i> ` + t("saves.manualActive", "Manual Override Active") + `</span>`;
            } else {
              pathStatus.innerHTML = isBP ? `<span style="color:#66c0f4;"><i class="fa-solid fa-robot"></i> ` + t("saves.autoActive", "Auto-Detected Folder") + `</span>` : `<span style="color:${colors.accent};"><i class="fa-solid fa-robot"></i> ` + t("saves.autoActive", "Auto-Detected Folder") + `</span>`;
            }
          } else {
            currentPathText.textContent = payload && payload.error ? payload.error : t("saves.notFound", "Save folder not detected automatically.");
            pathStatus.innerHTML = `<span style="color:#d0021b;"><i class="fa-solid fa-triangle-exclamation"></i> ` + t("saves.notConfigured", "Not Configured") + `</span>`;
          }
        } catch (err) {
          backendLog("LuaTools: Refresh Save Path parser error: " + err);
        }
      }).catch(function (err) {
        backendLog("LuaTools: GetSavePathsForApp call error: " + err);
      });
    }

    function refreshBackupsList() {
      if (isBP) {
        backupsList.innerHTML = `<div style="text-align:center;padding:24px 12px;color:#8f98a0;font-size:12px;"><i class="fa-solid fa-spinner fa-spin" style="margin-right:6px;color:#66c0f4;"></i>` + t("saves.loadingBackups", "Scanning backups folder...") + `</div>`;
      } else {
        backupsList.innerHTML = `<div style="text-align:center;padding:20px 12px;color:${colors.textSecondary};font-size:12px;"><i class="fa-solid fa-spinner fa-spin" style="margin-right:6px;color:${colors.accent};"></i>` + t("saves.loadingBackups", "Scanning backups folder...") + `</div>`;
      }

      if (typeof Millennium === "undefined" || typeof Millennium.callServerMethod !== "function") {
        backupsList.innerHTML = `<div style="text-align:center;padding:24px 12px;color:#8f98a0;font-size:12px;">` + t("saves.noBackups", "No backups found for this game.") + `</div>`;
        return;
      }

      Millennium.callServerMethod("luatools", "ListSaveBackups", {
        appid: appid,
        contentScriptQuery: "",
      }).then(function (res) {
        try {
          const payload = typeof res === "string" ? JSON.parse(res) : res;
          if (payload && payload.success && payload.backups) {
            allBackups = payload.backups;
            renderFilteredBackups();
          } else {
            if (isBP) {
              backupsList.innerHTML = `<div style="text-align:center;padding:28px 12px;color:#8f98a0;font-size:12px;"><i class="fa-solid fa-circle-info" style="margin-right:6px;color:#66c0f4;"></i>` + t("saves.noBackups", "No backups found for this game.") + `</div>`;
            } else {
              backupsList.innerHTML = `<div style="text-align:center;padding:20px 12px;color:${colors.textSecondary};font-size:12px;"><i class="fa-solid fa-info-circle" style="margin-right:6px;color:${colors.accent};"></i>` + t("saves.noBackups", "No backups found for this game.") + `</div>`;
            }
          }
        } catch (err) {
          backendLog("LuaTools: List backups parsing error: " + err);
        }
      }).catch(function (err) {
        backendLog("LuaTools: ListSaveBackups call error: " + err);
      });
    }

    // Save Path Button Handler
    savePathBtn.onclick = function (e) {
      e.preventDefault();
      const pathValue = pathInput.value.trim();
      if (!pathValue) {
        ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), t("saves.emptyPath", "Please specify a valid folder path."));
        return;
      }

      Millennium.callServerMethod("luatools", "SetManualSavePath", {
        appid: appid,
        path: pathValue,
        contentScriptQuery: "",
      }).then(function (res) {
        try {
          ...
          if (payload && payload.success) {
            ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), t("saves.pathUpdated", "Save path updated successfully!"));
            refreshSavePath();
            refreshBackupsList();
          } else {
            const errMsg = payload && payload.error ? payload.error : t("saves.invalidPath", "Invalid path. Directory does not exist.");
            ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), errMsg);
          }
        } catch (err) {
          backendLog("LuaTools: SetManualSavePath parser error: " + err);
        }
      }).catch(function (err) {
        backendLog("LuaTools: SetManualSavePath call error: " + err);
      });
    };
    ...
    createBackupBtn.onclick = function (e) {
      ...
      if (isBP) {
        spinner.style.cssText = `background:radial-gradient(circle at 50% 0%, #1e2838 0%, #0f1622 100%);border:1px solid rgba(102, 192, 244, 0.2);border-radius:12px;padding:24px 32px;display:flex;flex-direction:column;align-items:center;gap:12px;box-shadow:0 12px 40px rgba(0,0,0,0.6);`;
        spinner.innerHTML = `<i class="fa-solid fa-spinner fa-spin" style="font-size:32px;color:#66c0f4;"></i><div style="font-size:14px;color:#fff;font-weight:600;">` + t("saves.backingUp", "Compressing save files...") + `</div>`;
      } else {
        spinner.style.cssText = `background:${colors.modalBg};border:1px solid ${colors.border};border-radius:12px;padding:20px 28px;display:flex;flex-direction:column;align-items:center;gap:10px;box-shadow:0 8px 30px rgba(0,0,0,0.4);`;
        spinner.innerHTML = `<i class="fa-solid fa-spinner fa-spin" style="font-size:28px;color:${colors.accent};"></i><div style="font-size:13px;color:${colors.text};font-weight:600;">` + t("saves.backingUp", "Compressing save files...") + `</div>`;
      }
      
      loadingOverlay.appendChild(spinner);
      document.body.appendChild(loadingOverlay);

      Millennium.callServerMethod("luatools", "CreateSaveBackup", {
        appid: appid,
        contentScriptQuery: "",
      }).then(function (res) {
        try {
          ...
          const payload = typeof res === "string" ? JSON.parse(res) : res;
          if (payload && payload.success) {
            ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), t("saves.backupSuccess", "Backup created successfully!"));
            refreshBackupsList();
          } else {
            const errMsg = payload && payload.error ? payload.error : t("saves.backupFail", "Backup failed.");
            ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), errMsg);
          }
        } catch (_) {
          loadingOverlay.remove();
        ...
        loadingOverlay.remove();
        ShowLuaToolsAlert(t("saves.title", "Saves Backup & Restore"), String(err));
      });
    };
    ...
    // Scan Gamepad Focusable Elements (Only in Big Picture)
    if (isBP) {
      setTimeout(function () {
        if (window.GamepadNav) window.GamepadNav.scanElements();
      }, 150);
  ...
  function ensureStyles() {
    if (!document.getElementById("luatools-spacing-styles")) {
      ...
      style.textContent = `
                .luatools-restart-button, .luatools-button {
                ...
                .luatools-restart-button:hover, .luatools-button:hover {
                    background: rgba(102, 192, 244, 0.25) !important;
                    border-color: #66c0f4 !important;
                    color: #fff !important;
                    box-shadow: 0 0 8px rgba(102, 192, 244, 0.4) !important;
                }
                .luatools-restart-button span, .luatools-button span {
  ...
  function addLuaToolsButton() {
    ...
    if (
      ...
      function triggerHeaderAction(e) {
        e.preventDefault();
        e.stopPropagation();
        backendLog("Header button clicked");
        showSettingsPopup();
      }

      headerBtn.addEventListener("click", triggerHeaderAction);
      headerBtn.addEventListener("keydown", function (e) {
        if (e.key === "Enter" || e.key === " " || e.key === "GamepadA") {
          triggerHeaderAction(e);
        }
      });
