...
from saves import (
    get_save_paths_for_app,
    set_manual_save_path,
    create_save_backup,
    list_save_backups,
    restore_save_backup,
)
...
def GetSavePathsForApp(appid: int, contentScriptQuery: str = "") -> str:
    try:
        res = get_save_paths_for_app(appid)
        return json.dumps(res)
    except Exception as exc:
        logger.warn(f"LuaTools: GetSavePathsForApp failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def SetManualSavePath(appid: int, path: str, contentScriptQuery: str = "") -> str:
    try:
        res = set_manual_save_path(appid, path)
        return json.dumps(res)
    except Exception as exc:
        logger.warn(f"LuaTools: SetManualSavePath failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def CreateSaveBackup(appid: int, contentScriptQuery: str = "") -> str:
    try:
        res = create_save_backup(appid)
        return json.dumps(res)
    except Exception as exc:
        logger.warn(f"LuaTools: CreateSaveBackup failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def ListSaveBackups(appid: int, contentScriptQuery: str = "") -> str:
    try:
        res = list_save_backups(appid)
        return json.dumps(res)
    except Exception as exc:
        logger.warn(f"LuaTools: ListSaveBackups failed: {exc}")
        return json.dumps({"success": False, "error": str(exc), "backups": []})


def RestoreSaveBackup(appid: int, filename: str, contentScriptQuery: str = "") -> str:
    try:
        res = restore_save_backup(appid, filename)
        return json.dumps(res)
    except Exception as exc:
        logger.warn(f"LuaTools: RestoreSaveBackup failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
